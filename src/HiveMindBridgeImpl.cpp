...
HiveMindBridgeImpl::HiveMindBridgeImpl(
    ITCPServer &tcpServer, IHiveMindHostSerializer &serializer,
    IHiveMindHostDeserializer &deserializer, IMessageHandler &messageHandler,
    IThreadSafeQueue<MessageDTO> &inboundQueue,
    IThreadSafeQueue<OutboundRequestHandle> &outboundQueue, ILogger &logger)
    : m_tcpServer(tcpServer), m_serializer(serializer),
      m_deserializer(deserializer), m_messageHandler(messageHandler),
      m_inboundQueue(inboundQueue), m_outboundQueue(outboundQueue),
      m_logger(logger) {}
  ...
  if (isTCPClientConnected()) {
    ...
    for (auto result = m_inboundRequestsQueue.begin();
      ...
      if (result->getCallbackReturnContext().valid()) {
        if (result->getCallbackReturnContext().wait_for(
      ...
      if (request) {
        ...
        if (search != m_inboundResponsesMap.end()) {
          ...
          if (handle.getState() == OutboundRequestState::READY) {
            ...
            if (handle.bumpDelaySinceSent(THREAD_SLEEP_MS) >=
                DELAY_BRFORE_DROP_S) {
        ...
        m_logger.log(LogLevel::Warn,
                     "Outbound queue contains an unsupported message");
      }
    }
...
void HiveMindBridgeImpl::sendReturn(InboundRequestHandle result) {
  std::optional<CallbackReturn> callbackReturnOpt =
      result.getCallbackReturnContext().get();
...
bool HiveMindBridgeImpl::greet() {
  if (isTCPClientConnected()) {
    ...
    std::optional<uint32_t> swarmAgentId =
        m_messageHandler.handleGreet(greetResponse);

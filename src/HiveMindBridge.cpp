...
HiveMindBridge::HiveMindBridge(int tcpPort, ILogger &logger)
    : m_tcpServer(tcpPort, logger), m_deserializer(m_tcpServer),
      m_serializer(m_tcpServer), m_messageHandler(logger), m_logger(logger) {
  m_bridge = std::make_unique<HiveMindBridgeImpl>(
      m_tcpServer, m_serializer, m_deserializer, m_messageHandler,
...
void HiveMindBridge::onConnect(std::function<void()> hook) {
  m_bridge->onConnect(hook);
}

void HiveMindBridge::onDisconnect(std::function<void()> hook) {
  m_bridge->onDisconnect(hook);
}

...
MessageDTO MessageHandler::handleFunctionListLengthRequest(
    uint32_t requestId, uint32_t msgDestinationId, uint32_t msgSourceId,
    UserCallTargetDTO sourceModule) {

  uint32_t length = m_callbackNames.size();

  return MessageUtils::createFunctionListLengthResponseMessage(
      requestId, msgDestinationId, msgSourceId, sourceModule, length);
}
...
MessageDTO MessageHandler::handleFunctionDescriptionRequest(
  ...
  for (auto arg : manifest) {
    FunctionDescriptionArgumentDTO argument(arg.getName().c_str(),
                                            arg.getType());
    args.push_back(argument);
  }

  FunctionDescriptionDTO functionDescription(name.c_str(), args.data(),
                                             args.size());
...
MessageHandler::handleMessage(MessageDTO message) {
  ...
  if (std::holds_alternative<RequestDTO>(request)) {
    ...
    if (std::holds_alternative<UserCallRequestDTO>(userCallRequest)) {
      ...
      if (std::holds_alternative<FunctionCallRequestDTO>(functionCallRequest)) {
        ...
        GenericResponseStatusDTO responseStatus =
            GenericResponseStatusDTO::BadRequest;
        if (callback) {
          std::shared_future<std::optional<CallbackReturn>> ret =
              std::async(std::launch::async, callback.value(), functionArgs,
                         argsLength)
                  .share();
      ...
      } else if (std::holds_alternative<FunctionListLengthRequestDTO>(
                     functionCallRequest)) {
        result.setResponse(handleFunctionListLengthRequest(
            requestId, msgDestinationId, msgSourceId, sourceModule));
        // FunctionDescriptionRequest
      } else if (std::holds_alternative<FunctionDescriptionRequestDTO>(
                     functionCallRequest)) {
    ...
    if (std::holds_alternative<GenericResponseDTO>(vResponse)) {
      GenericResponseDTO genericResponse =
          std::get<GenericResponseDTO>(vResponse);
      ...
      UserCallResponseDTO userCallResponse =
          std::get<UserCallResponseDTO>(vResponse);
      ...
      if (std::holds_alternative<GenericResponseDTO>(vUserCallResponse)) {
        ...
        return InboundResponseHandle(response.getId(),
                                     genericResponse.getStatus(),
                                     genericResponse.getDetails());
      } else if (std::holds_alternative<FunctionCallResponseDTO>(
                     vUserCallResponse)) {
        FunctionCallResponseDTO functionCallResponse =
            std::get<FunctionCallResponseDTO>(vUserCallResponse);

        return InboundResponseHandle(
            response.getId(), functionCallResponse.getResponse().getStatus(),
      ...
      m_logger.log(LogLevel::Warn,
                   "Cannot handle response : unknown response type");
      return {};
    }
  } else {
    m_logger.log(LogLevel::Warn,
                 "Cannot handle message : unknown message type");
    return {};
  }
}
...
std::optional<CallbackFunction>
MessageHandler::getCallback(const std::string &name) {
